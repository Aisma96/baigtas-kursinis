...
def add_recipe():
            ...
            if product_key in request.form and quantity_key in request.form:
                ...
                if product and quantity:
                    try:
                        ...
                        if product:
                            calories =  float(str(product.calories).replace(',', '.'))
                            protein = float(str(product.protein).replace(',', '.'))
                            carbohydrate = float(str(product.carbohydrates).replace(',', '.'))
                            fat = float(str(product.fat).replace(',', '.'))
    ...
    if form.validate_on_submit():
        recipe = Recipe(
            ...
            carbohydrates=total_nutrition['Carbohydrate'],
...
def view_recipe(recipe_id):
    ...
    total_nutrition = {
        ...
        'Carbohydrate': recipe.carbohydrates,
        'Fat': recipe.fat
    }
    return render_template('view_recipe.html', recipe=recipe, ingredients=ingredients, total_nutrition=total_nutrition)
